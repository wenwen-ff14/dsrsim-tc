...
- Build: `dotnet build AnoMech.sln -c Release -p:RestoreLockedMode=true` from the repository root. Output is `AnoMech/bin/x64/Release/AnoMech.dll`.
- The csproj uses `Dalamud.NET.Sdk/13.0.0`, .NET 9 and C# 13. It prefers the Traditional Chinese SDK at `%AppData%/FFXIVSimpleLauncher/Dalamud/Injector/`; override with `DALAMUD_HOME` or `-p:DalamudLibPath=...`. Build rejects a Dalamud assembly outside API 13. Native compatibility bindings still require in-game verification on the target client.
