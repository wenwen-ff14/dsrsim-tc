...
// enforces a centered circle or square. Geometry is
...
internal sealed unsafe class SimArenaBoundary : ISimObject
{
    ...
    internal SimArenaBoundary(SimParty party, SimWorld world, float radius, string cause, bool showVfx = true, bool square = false)
    {
        this.party = party;
        ...
        if (showVfx && !square && Plugin.DataManager.FileExists(RingVfxPath))
    ...
    // Shared by the per-frame fence and teleport-to-spawn on reset.
    internal bool IsOutside(Vector3 local) => square
        ? local.X * local.X > radiusSq || local.Z * local.Z > radiusSq
