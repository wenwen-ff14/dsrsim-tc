...
public sealed partial class DsrP6DragonsScenario(DsrP6Section section=DsrP6Section.Breath1) : IScenario
{
    public string Name=>section switch
    {
    ...
    {
        if(section is DsrP6Section.Full or DsrP6Section.Wroth)
            throw new InvalidOperationException("十字火球尚未取得可核對的事件資料，暫不開放此段與全流程。");
        world=simWorld;state=new(validationSeed??Random.Shared.Next());
        actors.Clear();tethers.Clear();puddles.Clear();puddleHits.Clear();
        (startAt,endAt)=section switch
        {
            DsrP6Section.Breath1=>(0,35),DsrP6Section.Wings1=>(35,57),
            DsrP6Section.Wroth=>(58,104),DsrP6Section.Wings2=>(105,125),
            DsrP6Section.Breath2=>(126,162),_=>(0,162),
        };
        ...
        if(startAt>0)
        {
            ApplyVow(section switch{DsrP6Section.Wroth=>0,DsrP6Section.Wings2=>1,DsrP6Section.Breath2=>state.FirstVow==4?5:4,_=>state.FirstVow},
                section switch{DsrP6Section.Wings1=>21.109f,DsrP6Section.Wings2=>19.264f,DsrP6Section.Breath2=>32.334f,_=>34.1f});
            if(startAt>90)
            {
        ...
        world.Events.Add(50f,()=>{RestoreDragons();state.SetVowPass(0);});
        ...
        world.Events.Add(64.468f,()=>nidhogg?.Cast(27974,world.Party.Get(1)?.Position,7.7f,targetId:world.Party.Get(1)?.GameObjectId,fireDelay:.267f));
        ...
        world.Events.Add(78.874f,()=>{RestoreDragons();state.SetFlames();nidhogg?.Cast(27949,castSeconds:5.2f,fireDelay:.261f);});
        world.Events.Add(85.365f,()=>ResolveHot(false));
        world.Events.Add(86.212f,ResolveFlames);
        world.Events.Add(86.4f,()=>state.SetVowPass(1));
        ...
        world.Events.Add(154.044f,()=>{nidhogg?.SetPosition(Vector3.Zero);hraesvelgr?.SetPosition(new Vector3(0,0,25));nidhogg?.Cast(27968,castSeconds:0);hraesvelgr?.Cast(27968,castSeconds:0);});
        ...
        if(section==DsrP6Section.Breath1)world.Events.Add(35f,Complete);
        if(section==DsrP6Section.Wings1)world.Events.Add(57f,Complete);
        if(section==DsrP6Section.Wroth)world.Events.Add(104f,Complete);
        if(section==DsrP6Section.Wings2)world.Events.Add(125f,Complete);
    ...
    {
        var enemy=world!.SpawnEnemy(new EnemySpawnConfig(npc,NameId:name,Level:90,EnemyList:visible?EnemyListMode.ScenarioVisible:EnemyListMode.Never,IsVisible:visible,Placement:new(position,0),DisableLookAt:true));
