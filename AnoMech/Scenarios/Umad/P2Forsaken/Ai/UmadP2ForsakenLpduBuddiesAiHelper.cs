...
// Movement choreography for the "[LPDU] Buddies" strat — a copy of the EU "p3Z Buddy Meow"
// helper (UmadP2ForsakenP3ZBuddyMeowAiHelper), free to diverge without touching it. See the
// original helper for the framework design notes.
public sealed class UmadP2ForsakenLpduBuddiesAiHelper
{
    ...
    // First tower set only (i == 0): role decides the side — supports (tank/healer,
    // roles 0-3) take the right spot, DPS (roles 4-7) the left, regardless of whether
    // their group drew cones or chariots. This is the [LPDU] Buddies change (the p3Z
    // Buddy Meow original picked the side by cone/chariot instead). Every later tower
    // keeps the standard alpha-order ruling (#0 -> right, #1 -> left).
    private (PartyRole right, PartyRole left) StackSpots(int i)
    {
        ...
        return (int)first < 4 ? (first, second) : (second, first); // support -> right
    }
    ...
    public static void KroxyReorder(UmadP2ForsakenLpduBuddiesAiHelper helper, int index, IList<PartyRole> active)
    {
    }
}
